...
- `src/collector/import_resolver.rs` — **the actual shared resolver already in the codebase**: `resolve_imports()` turns raw import strings into real file paths for the import graph, and its crate-visible `resolve_specifier()` is reused as-is by inheritance resolution (M7) — the comment there is explicit that both must "use the exact same candidate rules." This is the mechanism call-graph resolution should extend or reuse, not reinvent. **Extraction is a two-stage pipeline**: `import_query` (`complexity/lang_dispatch.rs`) yields specifiers, and `candidates_for` here turns them into repo paths. Both stages now cover the same seven languages (Rust, JS/TS, Python, Go, Java, C#, Kotlin), pinned by `every_language_with_an_import_query_can_resolve_imports`. They drifted once — Kotlin had a query but no resolver arm, so its specifiers resolved to nothing and its import graph was always empty. Key off `resolves_imports(ext)`, the end-to-end capability, rather than the query alone, so call-graph work cannot reintroduce that gap.
- `src/metrics/complexity/inheritance.rs` — the closest existing precedent for resolving a name reference to a cross-file *definition* once a specifier is resolved (`extends Base` → the class that declares it), built on top of `import_resolver.rs` above. The result type is `BaseRef::{SameFile, Resolved{path,name}, Unresolvable}`, defined in `src/snapshot/mod.rs` (it's part of the cached snapshot schema) and terminates gracefully on anything it can't resolve (external packages, re-export chains, ambiguity). **Load-bearing detail: this definition-resolution layer is only wired up for TS/JS (`Language::JsTs`), not Rust or any of the other 6 languages**, even though it's a simpler problem than call resolution. That scoping decision is a real constraint you should reason about, not route around.
