...
- `src/collector/import_resolver.rs` — **the actual shared resolver already in the codebase**: `resolve_imports()` turns raw import strings into real file paths for the import graph, and its crate-visible `resolve_specifier()` is reused as-is by inheritance resolution (M7) — the comment there is explicit that both must "use the exact same candidate rules." This is the mechanism call-graph resolution should extend or reuse, not reinvent. **Extraction is a two-stage pipeline and the stages do not agree on which languages are covered**: `import_query` (`complexity/lang_dispatch.rs`) yields specifiers for Rust, JS/TS, Python, Go, Java, C# *and Kotlin*, but `candidates_for` here has no Kotlin arm — Kotlin specifiers resolve to nothing, so a Kotlin repository's import graph is always empty. `resolves_imports(ext)` exposes the narrower, end-to-end capability; key off that, not off the query, or call-graph work will inherit the same gap.
