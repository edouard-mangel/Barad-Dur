  ...
  /* Static intro: what the cards show and how quality is scored. */
  function acIntro() {
    ...
    var explainer = buildExplainer('Understanding commit quality and author metrics', [
      {
  ...
  function acActivityColor(days) {
    if (days <= 30) return 'var(--c-good)';
    if (days <= 90) return 'var(--c-warn)';
    return 'var(--c-danger)';
  }

  function acQualityColor(q) {
    if (q >= 70) return 'var(--c-good)';
    if (q >= 40) return 'var(--c-warn)';
    return 'var(--c-danger)';
  }

  /* Cards matching `query` (name/email substring), ordered by `sortKey`. */
  function acFilterSort(cards, query, sortKey) {
    ...
    filtered.sort(function(a, b) {
      if (sortKey === 'commits') return b.commit_count - a.commit_count;
      if (sortKey === 'files') return b.files_owned - a.files_owned;
      if (sortKey === 'active') return a.days_since_active - b.days_since_active;
      if (sortKey === 'quality') return b.avg_commit_quality - a.avg_commit_quality;
      return 0;
    });
    return filtered;
  }

  /* One author report card. */
  function acCard(c) {
    var card = el('div', { className: 'ac-card' });

    var nameEl = el('div', { className: 'ac-name' });
    var badge = el('span', { className: 'ac-badge' });
    badge.style.backgroundColor = acActivityColor(c.days_since_active);
    ...
    var qFill = el('div', { style: { height: '100%', borderRadius: '3px', width: qRounded + '%', background: acQualityColor(qRounded) } });
    qBar.append(qFill);
    var qVal = el('div', { style: { fontSize: '11px', color: '#94a3b8', marginTop: '2px' } });
    qVal.append(txt(qRounded + '/100'));

    var activeEl = el('div', { style: { fontSize: '12px', color: acActivityColor(c.days_since_active), marginTop: '8px' } });
  ...
  function buildAuthorsTab() {
    ...
    acIntro().forEach(function(block) { container.append(block); });
    ...
    function renderCards() {
      var visible = acFilterSort(cards, search.value.toLowerCase(), currentSort);
      grid.replaceChildren.apply(grid, visible.map(acCard));
    }
  ...
  /* Header: brand, repo meta chips, palette/quick-open/theme buttons. */
  function appHeader() {
  ...
  /* Render one tab's content, degrading a throwing tab to an inline error
     placeholder instead of taking the whole report down. */
  function appSafeRender(name, builder) {
    try {
      return builder();
    } catch (e) {
  ...
  /* Tab bar + lazily-rendered content panels. Returns the pieces renderApp
     mounts, plus switchToTab for drill-through links. */
  function appBuildTabs() {
    ...
    function activate(idx) {
      tabs.querySelectorAll('.tab').forEach(function(tb) { tb.className = 'tab'; });
      contentDivs.forEach(function(cd) { cd.className = 'tab-content'; });
      tabs.querySelectorAll('.tab')[idx].className = 'tab active';
      contentDivs[idx].className = 'tab-content active';
      // Lazy-render on first visit
      if (contentDivs[idx].dataset.rendered !== '1') {
        contentDivs[idx].replaceChildren(appSafeRender(tabNames[idx], tabContents[idx]));
    ...
    tabNames.forEach(function(name, i) {
      ...
      tab.append(txt(name));
      contentDivs.push(el('div', { className: 'tab-content' + (i === 0 ? ' active' : '') }));
      tab.addEventListener('click', function() {
        activate(i);
        setHashState(tabNames[i]);
      });
      tabs.append(tab);
    });

    // Pre-render overview immediately
    contentDivs[0].replaceChildren(appSafeRender(tabNames[0], tabContents[0]));
    ...
    return { tabs: tabs, contentDivs: contentDivs, switchToTab: switchToTab };
  }
  ...
  function renderApp() {
    ...
    var built = appBuildTabs();

    document.addEventListener('barad:switch-tab', function(e) {
      built.switchToTab(e.detail.tab);
    });

    app.replaceChildren(header, built.tabs, ...built.contentDivs);
