...
mod tests {
    ...
    fn fan_out_coupling_detects_high_fanout() {
        ...
        for i in 0..6 {
            snapshot
                .file_change_pairs
                .push((hub.clone(), PathBuf::from(format!("p{}.rs", i)), 3));
        }
    ...
    fn temporal_coupling_scores_50_with_four_to_eight_pairs() {
        ...
        for i in 0..4usize {
            ...
            snapshot
                .commits_by_file
                .insert(a, (0..10).map(|j| format!("c{}_{}", i, j)).collect());
            snapshot
                .commits_by_file
                .insert(b, (0..10).map(|j| format!("d{}_{}", i, j)).collect());
        }
        let result = temporal_coupling(&snapshot);
    ...
    fn fan_out_coupling_scores_50_with_three_to_five() {
        ...
        for hub_idx in 0..3usize {
            ...
            for i in 0..6 {
                snapshot.file_change_pairs.push((
                    hub.clone(),
                    PathBuf::from(format!("h{}p{}.rs", hub_idx, i)),
                    2,
                ));
            }
        }
