...
- `src/collector/` — git snapshot collection: git2 for commits/files, parallel `git blame --porcelain` (rayon), tree-sitter AST complexity (8 languages). Two readers — the working tree (`snapshot_builder.rs`, parallel) and a commit's blobs (`collect_snapshot_at_with_ast`, sequential; gate baselines and backfill) — only acquire content (both list files from a commit's tree, HEAD's for the live reader, which then reads source and manifests from disk; the historical reader reads both from that commit's blobs); `source_assembly.rs` owns the shared `RawSourceChannels::aggregate` → `resolve` path (manifest reader passed explicitly, never a hidden disk fallback) and `assemble` builds every snapshot and its indexes once
...
A per-MR diff above `MUTATION_CAPACITY` mutants (default 100, what six 4h shards fit with their ~30min baselines at the slow end of the measured throughput) cannot be tested in CI at all: every shard hits the wall and no verdict comes out. The shards then write one `MUTATION OVER CAPACITY` line instead of running, and `mutation-gate` requires the MR description to carry a `Mutation gate override` section — the kill rate of a local `scripts/mutation-campaign.sh` run and the disposition of every survivor. On `main` there is no description to carry it, so the nightly sweep covers that case.
