...
- `src/collector/exclude.rs` + `ignore_file.rs` — file exclusion, applied once to the tracked file tree in `snapshot_builder.rs::should_include`. Three layers, highest first: (1) CLI `--exclude`/`--exclude-ext` force-exclude, (2) repo-root `.baraddurignore` (full `.gitignore` semantics via the `ignore` crate; `!` rules re-include default-excluded files), (3) built-in defaults in `exclude.rs` (toggled by `--no-default-excludes`). `is_excluded` in `exclude.rs` is the pure defaults/pattern matcher; `should_include` in `ignore_file.rs` composes the three layers
