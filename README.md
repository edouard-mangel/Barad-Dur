...
Finding-based metrics are scored against the size of the population being assessed. Below 100 production-source files the denominator is too small to trust, so absolute count bands apply. At 100 or more, the **stricter of prevalence and the absolute count bands** applies: no affected files scores 100, up to 1% scores at most 90, up to 5% at most 75, up to 20% at most 50, and above 20% scores 25. Retaining the count cap prevents a score from improving merely because unrelated files were added. Tests, documentation, configuration, and generated files are excluded where a metric is specifically about production maintainability.
